...
import {TransformCodeType} from '@/types/code'
...
watch(() => props.code, (val: string) => {
  // debugger
  transformCode[configForm._scriptType](val)
})
const transformCode:TransformCodeType = {
  vue2x:(source: string)=>{
  ...
  vue3x:(source: string) => {
    ...
    previewCode.value = importjsTemplete(url)
  }
