		...
		<el-form class="demo-ruleForm" ref="elFormRef" label-width="80px" size="mini" :model="ruleForm">
			<div class="flex flex-col p-4 bg-gray-500 odd:bg-gray-900" v-for="(form, index) in ruleForm" :key="index">
				<div class="flex items-start">
					...
					<el-button v-if="index !== 0" class="ml-4" type="danger" :icon="Delete" @click="delHandle(index)"></el-button>
				</div>
				<div class="flex">
...
const genHandle = () => {
	elFormRef.value?.validate((valid: boolean) => {
		if (valid) {
			...
			// prettier.format(code, {parse: 'vue'})
			// console.log(prettier.format(code, {parser: 'html', plugins: parserHtml}))
